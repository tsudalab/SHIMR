...
class reject_boost(object):
	def __init__(self, Y, d, n_bins, size_U, data_path, fname):		
		self.filename_dual=fname + '_dual_boost_L1_d_' + str(d) + '.lp'
		self.filename_primal=fname + '_primal_iBoost_L1_d_' + str(d) + '.lp'
		self.data_path=data_path
		self.Opt_dual=Optimizer(Y, d)
		self.Opt_Primal=Optimizer_Primal(Y, d)
		self.d=d
		self.n_bins=n_bins	
		self.size_U=size_U
		self.ir=None	
	...
	def solve_Dual(self, X, Y, d, C_POS, C_NEG):
		### Create an instance of "Reject_Boost_Trainer"
		trainer = Reject_Boost_Trainer(self.data_path,self.size_U)
		predictor = trainer.train(X, Y, d, C_POS, C_NEG, self.Opt_dual, self.filename_dual)	
		return predictor


	def solve_Primal(self, predictor, X, C_POS, C_NEG):
		buf_all=predictor.generate_Column_2(X, predictor.feature_set)		
		predictor._solve_LP_Primal(self.Opt_Primal, buf_all, C_POS, C_NEG, self.filename_primal)
	...
	def apply_model(self, predictor, X, Y,  FLAG):			
		if(predictor.feature_set):
			...
			# if(np.any(np.absolute(predictor.dual_values_updated)) == False):
			if(np.any(np.absolute(predictor.primal_values)) == False):
				...
				if(len(indx)>0):				
					...
					if(np.unique(Y[indx]).shape[0]>1):						
						fpr, tpr, _ = roc_curve(Y[indx], pred_proba_cal[indx])
						roc_auc = round(auc(fpr, tpr),2)
						print('roc_auc: '+ str(roc_auc))
						area_pr=round(average_precision_score(Y[indx], pred_proba_cal[indx]),2)
						print('area_pr: '+ str(area_pr))

					rr=round(((Y.shape[0] - len(indx) ) / Y.shape[0]),2)
					acc=round(accuracy_score(Y[indx],np.array(label_pred_all)[indx]),2)		
					...
					# title='DB vs RR (C1 vs C2) \n (rr= ' +str(rr) +  ', acc= ' +str(acc) + ', auc= '+str(roc_auc) +')'
					# plot_DB(predictor, X_orig, Y, self.n_bins, label_pred_all, self.ir, title, legend_all, label_all, 50, f_name)
